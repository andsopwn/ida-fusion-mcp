# ida-pro-mcp (upstream) vs ida-fusion-mcp — Comparison

Last updated: 2026-04-16 (upstream commit `d80ed7f`)

This document tracks what upstream features/fixes ida-fusion-mcp has NOT yet adopted.

## Current Coverage

After PRs #2–#8, ida-fusion-mcp has ported or implemented equivalents of most upstream tools. The remaining gaps are **bugfixes, optimizations, and minor features** rather than major missing tool categories.
...
**Current state**: ida-fusion-mcp uses raw `ida_bytes.get_bytes()` in `api_memory.py` and `api_types.py`, so reading BSS globals returns incorrect `0xFF` byte fills.
...
**Current state**: ida-fusion-mcp sends raw Hex-Rays/disasm output with original whitespace, inflating token cost.
...
**Current state**: ida-fusion-mcp uses default `json.dumps()` with spaces (`", ": "`).
...
**Current state**: ida-fusion-mcp's `parse_address()` only accepts hex/decimal strings. Passing a function name like `"main"` fails with an error.
...
**Current state**: ida-fusion-mcp calls `init_caches()` eagerly on plugin load (`__init__.py` exports it, plugin calls it on Ctrl+M).
...
**Current state**: ida-fusion-mcp uses `ida_major` version checks and separate `idalib_worker.py` architecture (subprocess model), so this is less relevant.
...
## ida-fusion-mcp Unique Features (Not in Upstream)
