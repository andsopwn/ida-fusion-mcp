"""Instance registry for ida-fusion-mcp.
...
def _validate_instance_entry(instance_id: str, entry: Any) -> bool:
    ...
    if not isinstance(entry, dict):
        print(f"[ida-fusion-mcp] Warning: invalid registry entry '{instance_id}': not a dict",
              file=sys.stderr)
        return False
    ...
    if not isinstance(port, int) or port < 1 or port > 65535:
        print(f"[ida-fusion-mcp] Warning: invalid port in registry entry '{instance_id}': {port!r}",
              file=sys.stderr)
        return False
    ...
    if not _is_loopback_host(host):
        print(f"[ida-fusion-mcp] Warning: non-loopback host in registry entry '{instance_id}': {host!r}",
              file=sys.stderr)
        return False
    ...
    if pid is not None and (not isinstance(pid, int) or pid < 0):
        print(f"[ida-fusion-mcp] Warning: invalid pid in registry entry '{instance_id}': {pid!r}",
              file=sys.stderr)
        return False
