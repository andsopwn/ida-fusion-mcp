"""IDA Pro plugin for ida-fusion-mcp.
...
The ida_mcp package is bundled with ida-fusion-mcp and provides all IDA tools.
"""
...
# Import registration functions (add parent to path for ida_fusion_mcp imports)
_pkg_dir = str(Path(__file__).parent.parent.parent)
if _pkg_dir not in sys.path:
    sys.path.append(_pkg_dir)

from ida_fusion_mcp.plugin.registration import (
    register_instance,
    unregister_instance,
    update_heartbeat,
    get_binary_metadata,
...
def _load_ida_mcp():
    """Load the ida_mcp package (bundled with ida-fusion-mcp).
    ...
    """
    from ida_fusion_mcp.ida_mcp import MCP_SERVER, IdaMcpHttpRequestHandler, init_caches
    return MCP_SERVER, IdaMcpHttpRequestHandler, init_caches


class IdaFusionMcpPlugin(idaapi.plugin_t):
    ...
    help = "Runs MCP server and registers with ida-fusion-mcp"
    ...
    def init(self):
        ...
        if _is_gui_runtime():
            print("[ida-fusion-mcp] Plugin loaded (PLUGIN_FIX)")
        else:
            print("[ida-fusion-mcp] Plugin loaded (headless mode, server managed externally)")
    ...
    def start_server(self):
        ...
        if self.mcp_server and self.mcp_server._running:
            print("[ida-fusion-mcp] Server already running")
            return

        print("[ida-fusion-mcp] Starting MCP server...")
        ...
        try:
            ...
            print(f"[ida-fusion-mcp] ERROR: ida_mcp package failed to load: {e}")
            print("[ida-fusion-mcp] This indicates a broken installation. Reinstall ida-fusion-mcp.")
            return
        ...
        try:
            ...
            print(f"[ida-fusion-mcp] Cache init warning: {e}")
        ...
        try:
            ...
            if self.mcp_server._http_server:
                ...
                print(f"[ida-fusion-mcp] Server listening on 127.0.0.1:{self.server_port}")

                # Update download base URL to reflect the actual dynamic port
                from ida_fusion_mcp.ida_mcp.rpc import set_download_base_url
                ...
                print(f"[ida-fusion-mcp] Registered as instance '{self.instance_id}' "
                ...
                print("[ida-fusion-mcp] Failed to get server port")

        except OSError as e:
            if e.errno in (48, 98, 10048):  # Address already in use
                print(f"[ida-fusion-mcp] Error: Port binding failed")
            else:
                print(f"[ida-fusion-mcp] Failed to start server: {e}")
                import traceback
                traceback.print_exc()
    ...
    def _heartbeat_loop(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                print(f"[ida-fusion-mcp] Heartbeat error: {e}")
    ...
    def stop_server(self):
        ...
        print("[ida-fusion-mcp] Stopping server...")
        ...
        if self.instance_id:
            try:
                ...
                print(f"[ida-fusion-mcp] Unregistered instance '{self.instance_id}'")
            except Exception as e:
                print(f"[ida-fusion-mcp] Failed to unregister: {e}")
            self.instance_id = None
    ...
    def term(self):
        ...
        print("[ida-fusion-mcp] Plugin terminating")
...
class IdbHooks(idaapi.IDB_Hooks):
    ...
    def closebase(self):
        ...
        print("[ida-fusion-mcp] Database closing")
...
class UiHooks(idaapi.UI_Hooks):
    ...
    def database_inited(self, is_new_database, idc_script):
        ...
        print("[ida-fusion-mcp] Database initialized")
        if _is_gui_runtime():
